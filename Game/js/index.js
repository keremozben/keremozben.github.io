...
function solve( gridObj ){
	...
	if(typeof gridObj !== 'undefined'){
		...
		if( x.length > 0 && checkHasAvailable( gridObj ) ){
			stepMoveCount.push( x );
			$(gridObj).text(stepMoveCount.length);
			solve( x[0] );
		} else {
			stepMoveCount[stepMoveCount.length-1][0].text( stepMoveCount.length+1 );
			if( $('.grid.active:empty').size() == 0 ){
				var str = '';
				for( var b = 0, n = $('.grid').size(); b < n; b++ ){
					str += $('.grid').eq(b).text() + ' ';
				}
				availableSolutionCount.push( str );
				console.log('Çözüldü ' + iterator);
				return false;
			}
			stepMoveCount[stepMoveCount.length-1].shift();
			$('.grid').filter(':contains('+(stepMoveCount.length+1)+')').empty();
			if( stepMoveCount[stepMoveCount.length-1].length > 0 ){				
				solve( stepMoveCount[stepMoveCount.length-1][0] );
			} else {
				stepMoveCount.pop();
				
				for( var k = stepMoveCount.length; k > 0; k-- ){
					if( stepMoveCount[stepMoveCount.length-1].length == 1 ){
						$('.grid').filter(':contains('+(stepMoveCount.length+1)+')').empty();
						stepMoveCount.pop();
					} else {
						break;
