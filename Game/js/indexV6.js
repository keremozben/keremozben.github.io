...
var Game = {
	...
	getAvailableGrids : function(grid,writeable){
		...
		if( Game.matrix[grid[0]][grid[1]] == 0 ){
			return temp;
		}
...
function solveAll(){
	for(var zi = 0, bi = Game.matrix.length; zi < bi; zi++){
		for(var za = 0, ba = Game.matrix[zi].length; za < ba; za++){
			solve( [zi,za], stepMoveCount );
		}
	}
}

function solve( array, sols ){
	var
	...
	do{
		if( sols.length == 0 ){
			grid = array;
			x = Game.getAvailableGrids( array );
		} else {
			grid = false;
			...
			if(sols.length > 0){
				x = Game.getAvailableGrids( sols[sols.length-1][0] );
			} else {
				console.log('Bitti');
